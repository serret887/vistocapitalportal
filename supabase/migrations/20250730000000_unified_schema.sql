...
-- Ensures each user can only have one client per email address
COMMENT ON CONSTRAINT clients_user_email_unique ON clients IS 'Ensures each user can only have one client per email address';
